...
public class Reusable_Actions_PageObjects {
    ...
    //Reusable Method to submit keys on any webelement by explicit wait
    public static void submitKeysOnElement(WebDriver driver, WebElement xpathLocator, String userValue, ExtentTest logger, String elementName){
        //define by explicit wait
        WebDriverWait wait = new WebDriverWait(driver, 10);
        //use try catch to locate an element and click on it
        try{
            ...
            element.submit();
            logger.log(LogStatus.PASS, "Successfully submit keys on "+ elementName);
        } catch (Exception e){
            System.out.println("Unable to submit on element " + elementName + " " + e);
            getScreenShot(driver,elementName,logger);
            logger.log(LogStatus.FAIL, "Unable to submit on element " + elementName + " " + e);
    ...
    //Method for uploading files to webdriver
